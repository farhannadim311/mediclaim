...
// Initialize service
const apiService = new SimpleAPIService(logger);
...
app.post("/api/wallet/connect", async (req, res) => {
  try {
    ...
    const walletConnection = await apiService.connectWallet(seed);
...
app.post("/api/wallet/generate", async (req, res) => {
  try {
    const walletConnection = await apiService.generateWallet();
    res.json(walletConnection);
  } catch (error) {
    const errorMessage =
...
app.post("/api/contract/deploy", async (req, res) => {
  try {
    ...
    const result = await apiService.deployContract(walletSeed);
    res.json(result);
  } catch (error) {
    const errorMessage =
...
app.post("/api/contract/join", async (req, res) => {
  try {
    ...
    const result = await apiService.joinContract(walletSeed, contractAddress);
    res.json(result);
  } catch (error) {
    const errorMessage =
...
app.post("/api/claims/verify", async (req, res) => {
  try {
    ...
    const result = await apiService.verifyClaim(claimData);
    res.json(result);
  } catch (error) {
    const errorMessage =
...
app.get("/api/claims/:walletSeed", async (req, res) => {
  try {
    ...
    const claims = await apiService.getClaims(walletSeed);
    res.json(claims);
  } catch (error) {
    const errorMessage =
...
app.get("/api/claims/:walletSeed/:claimId", async (req, res) => {
  try {
    ...
    const claim = await apiService.getClaimStatus(walletSeed, claimId);
    res.json(claim);
  } catch (error) {
    const errorMessage =
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const state = await apiService.getContractState(
        walletSeed,
        contractAddress
      );
      res.json(state);
...
server.listen(port, () => {
  logger.info({ port }, "Simple API Bridge server started");
  logger.info(`Server running at http://localhost:${port}`);
  logger.info("Health check: GET /health");
  logger.info("This bridge provides CLI-like functionality for the UI");
});
