...
export function buildClaimVerificationMessage(
  F: any,
...
export async function signClaimVerification(params: {
  skHex: string; // 32-byte scalar hex with/without 0x
  claimHash: Uint8Array;
  providerId: Uint8Array;
  patientId: Uint8Array;
  amount: bigint;
  serviceDate: bigint;
}): Promise<{ R8x: Uint8Array; R8y: Uint8Array; S: Uint8Array }> {
  const babyJub = await circomlib.buildBabyjub();
  const eddsa = await circomlib.buildEddsa();
  const F = babyJub.F;

  let skHex = params.skHex;
  if (skHex.startsWith("0x") || skHex.startsWith("0X")) skHex = skHex.slice(2);
  if (skHex.length !== 64) throw new Error("skHex must be 32 bytes hex");

  // Use Buffer directly like claim-witnesses
  const sk = Buffer.from(skHex, "hex");

  const msgArray = buildClaimVerificationMessage(
    F,
    params.claimHash,
    params.providerId,
    params.patientId,
...
export function validateProviderAuthorization(
  ...
  if (!authorizedClaimTypes) return false;
  return authorizedClaimTypes.has(claimType);
}

// Check if patient is eligible
export function validatePatientEligibility(
  patientId: string,
